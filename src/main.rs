...
struct ThreadPoolSharedData {
    //thread name
    name: Option<String>,
    //use it to store the receiver of the channel
...
impl ThreadPool {
    ...
    //wait at the main thread, until all jobs are done
    pub fn join(&self) {
...
fn spawn_in_pool(shared_data: Arc<ThreadPoolSharedData>) {
    ...
    builder.spawn(move || {
        ...
        loop {
            //使用的内存顺序为Acquire，代表load方法能看到之前所有线程对active_count所做的修改
...
//monitor the jobs execution situation
struct Sentinel<'a> {
